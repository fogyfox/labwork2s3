...
//рекурсивная функция для проверки возможности разбиения
bool canPartitionIntoSubsets(const vector<int>& nums, int k, int targetSum, int currentIndex, vector<int>& subsetSums, vector<bool>& used, vector<vector<int>>& result) {
    //все подмножества заполнены
    if (subsetSums.size() == k) {
        //проверяем, что все суммы равны targetSum
    ...
    //если текущая сумма подмножества достигла целевой, начинаем новое подмножество
    if (!subsetSums.empty() && subsetSums.back() == targetSum) {
        vector<int> newSubsetSums = subsetSums;
        newSubsetSums.push_back(0);
        return canPartitionIntoSubsets(nums, k, targetSum, 0, newSubsetSums, used, result);
    }
    
    //перебираем элементы для добавления в текущее подмножество
    for (int i = currentIndex; i < nums.size(); i++) {
        if (!used[i] && subsetSums.back() + nums[i] <= targetSum) {
            used[i] = true;
            subsetSums.back() += nums[i];
            
            if (canPartitionIntoSubsets(nums, k, targetSum, i + 1, subsetSums, used, result)) {
                //восстанавливаем результат
                if (result.size() < subsetSums.size()) {
                    result.push_back(vector<int>());
                }
                result[subsetSums.size() - 1].push_back(nums[i]);
                return true;
            }
...
//основная функция разбиения множества
bool partitionSet(const Set* set, int subsetSum, vector<vector<int>>& result) {
    ...
    return canPartitionIntoSubsets(nums, k, subsetSum, 0, subsetSums, used, result);
}

//альтернативная реализация с backtracking
...
//улучшенная версия функции разбиения
bool partitionSetImproved(const Set* set, int subsetSum, vector<vector<int>>& result) {
    ...
    //проверяем возможность разбиения
    if (totalSum % subsetSum != 0) {
        cout << "Невозможно разбить множество: общая сумма " << totalSum << " не делится нацело на " << subsetSum << endl;
        return false;
    }
    ...
    if (nums[0] > subsetSum) {
        cout << "Невозможно разбить множество: элемент " << nums[0] << " превышает целевую сумму " << subsetSum << endl;
        return false;
    }
